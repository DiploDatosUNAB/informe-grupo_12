...
# El campo dia_laborable contenía valores con acentos y no encontré la forma de trabajarlos, así que probando encontré una forma de colapsarlos a un caracter que pudiese trabajar
```
...
base_cruda %>% 
  mutate(suma_primer_bloque = trabajo_pago + trabajo_domestico_no_pago + voluntariados + estudio_educacion + recreacion + aplicaciones + tareas_cuidados + cuidado_personal) %>% 
  summarise(mean(suma_primer_bloque))
...
base_cruda %>% 
  mutate(suma_primer_bloque = trabajo_pago + trabajo_domestico_no_pago + voluntariados + estudio_educacion + recreacion + aplicaciones + tareas_cuidados + cuidado_personal) %>% 
  summarise(var(suma_primer_bloque))
```
...
ggplot(base_con_promedios, aes(suma_primer_bloque)) +
  geom_histogram(bins = 10, aes(colour=as.factor(sexo)))
...
base_cruda %>% 
  mutate(suma_segundo_bloque = trabajo_pago2 + trabajo_domestico_no_pago2 + voluntariados2 + estudio_educacion2 + recreacion2 + aplicaciones2 + tareas_cuidados2 + cuidado_personal2) %>% 
  summarise(mean(suma_segundo_bloque))
```
...
base_cruda %>% 
  mutate(suma_segundo_bloque = trabajo_pago2 + trabajo_domestico_no_pago2 + voluntariados2 + estudio_educacion2 + recreacion2 + aplicaciones2 + tareas_cuidados2 + cuidado_personal2) %>% 
  summarise(var(suma_segundo_bloque))
```
...
Primera aproximación a la Hipótesis 3:
```{r}
#Promedio de hs. de trabajo pago en hombres
base_cruda %>% 
  filter(sexo == "Varon") %>% 
  filter(dia_laborable == "Dma laborable") %>% 
  summarise(mean(trabajo_pago))
```


```{r}
#Promedio de hs. de trabajo pago en mujeres
base_cruda %>% 
  filter(sexo == "Mujer") %>% 
  filter(dia_laborable == "Dma laborable") %>% 
  summarise(mean(trabajo_pago))
