...
### Hipótesis 1: Producto de las asimetrías de género, las mujeres dedican más tiempo al trabajo no remunerado y a las tareas de cuidado que los hombres.
...
```
En efecto, las mujeres dedican en promedio 2.5 hs más que los hombres al trabajo no remunerado y a las tareas de cuidado. Esto verificaría la existencia de asimetrías de género.


### Hipótesis 2: Las personas menores de 18 años dedican más tiempo a las actividades recreativas que el resto de las personas mayores de 18 años empleadas en un día laborable.

```{r}
#Tiempo dedicado a actividades recreativas de menores de 18 años
base_cruda %>% 
  filter(edad < "19") %>% 
...
```{r}
#Tiempo dedicado a actividades recreativas de mayores de 18 años empleadas en un día laborable
base_cruda %>% 
  filter(edad > "18", dia_laborable == "Día laborable", estado_ocupacional == "Ocupado") %>%
  summarise("tiempo_de_recreacion_+_18" = mean(recreacion))
```
Como era de esperar, las personas menores de 18 años dedican en los días laborables más tiempo (el doble aproximadamente) a las actividades recreativas que las personas empleadas mayores de 18 años. Esto se debería a que al no necesitar trabajar, las personas menores de 18 años cuentan con más tiempo libre disponible para dedicar a actividades recreativas.



### Hipótesis 3: Producto de las asimetrías de género, las mujeres activas dedican menos tiempo al trabajo remunerado que los hombres activos.
...
```
Aquí también se verifica la hipótesis. Las mujeres le dedican poco más de 1 hora menos por día al trabajo pago que los hombres.
